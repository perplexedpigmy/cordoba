 ...
 * - Cache entries are serialized, multiple threads can use the same cached
 *repositories simultaneously.
 * - On shutdown open repositories are released.
 * - [Experimental] Support for thread connection. Once a thread connects to a
 *repository, there is no need to pass the connection as a argument to lower
 *levels of the application, it's possible to use thread local storage to access
 *the thread context IMPORTANT: thread local storage is experimental, some of
 *the semantics may change It was introduced to allow disjoint access to a
 *context within the same thread, without the need for sharing a connection but
 *the required semantics are not yet thought through, and emerging semantics a.
 *Should the user allow to close it b. What should happen on failure, failed
 *context is a dead-end, so it should be reset, by user? automatically? c. How
 *should it work with same task on multiple threads
 *   ...
 *   In short it's namely experimental, to identify emerging semantics.
 *
 *  NOTE: While only one instance is in use, It is not a singleton by design.
 *  In most applications the singleton is superfluous, and in many applications
 *it's limiting, or even hazardous (= global static) While code should be as
 *explicit as possible. The cardinality requirements were not yet fully flushed
 *out.
 **/
class GitAccess {
public:
  ...
  /// @param ...args path, repository pair to be kept in a hash table
  /// @return A pointer to repository_t, the pointer is guaranteed to be safe to
  /// use as repositories are open for the entire life time of the application.
  template <typename... Ts> gd::repository_t *cacheRepo(Ts &&...args) {
    std::lock_guard<std::shared_mutex> lock(cacheAccess_);
  ...
  /// @return a pointer to an already opend repository, or an error, when such
  /// repository wasn't opened/created yet.
  Result<gd::repository_t *> getRepo(const std::string &repoFullPath) {
  ...
  /// @return The context of the thread, or an Error if the context failed
  /// anywhere in the previous calls
  Result<gd::Context> threadContext() const noexcept {
    if (not ctx_.repo_)
      return gd_unexpected(gd::ErrorType::MissingRepository,
                           sNoRepositoryError);
  ...
  /// @param fullPathRef The full path reference that can be found in the `refs`
  /// directory
...
static std::string sBranchRefRoot{
    "refs/heads/"}; // Relative location to git references
static std::string sHead{
    gd::defaultRef}; // Default reference when not indicated by user
thread_local gd::Context GitAccess::ctx_{
    nullptr}; // Implicit context per thread

static std::shared_ptr<spdlog::logger> sLogger{
    spdlog::null_logger_mt("No Logger")};

/**
 * Create a repository at a given path with a given name
 * The repository is owned by the cache and as long as the cache is alive (No
 *cleanup was called) It is safe to use the the raw pointer to the git
 *repository.
 *
 * The cache is introduced to avoid paying for git repository opening for every
 *call as well as support a default context, when no user context is defined the
 *commands will be executed in the last valid context.
 *
 ...
 **/
/// @brief tests for repository existence on file system
/// @param repoPath full path of a repository on a files system
/// @return True if a git repository exists in `repoPath` location otherwise,
/// false.
bool repoExists(const std::filesystem::path &repoPath) noexcept {
  return git_repository_open_ext(nullptr, repoPath.c_str(),
...
gd::ObjectUpdate::createBlob(gd::Context &ctx,
                             const std::filesystem::path &fullpath,
                             const std::string &content) noexcept {
  ObjectUpdate blob{
      create(fullpath, GIT_FILEMODE_BLOB, &gd::ObjectUpdate::insert)};
  if (git_blob_create_from_buffer(&blob.oid_, *ctx.repo_, content.c_str(),
                                  content.size()) != 0)
...
Result<gd::ObjectUpdate>
gd::ObjectUpdate::fromEntry(gd::Context &ctx,
                            const std::filesystem::path &fullpath,
...
gd::ObjectUpdate::remove(const std::filesystem::path &fullpath) noexcept {
  ObjectUpdate removed{create(fullpath, GIT_FILEMODE_UNREADABLE /* Ignored */,
                              &gd::ObjectUpdate::remove)};

  sLogger->debug("Removed {}", fullpath);
  return std::move(removed);
}

Result<void> gd::ObjectUpdate::gitIt(git_treebuilder *bld) const noexcept {
...
/// @param fullpath full path of the directory in the repository include its
/// name
/// @param bld A Builder for the Tree (=dir) the new directory is to be added to
/// @return On success an Object representation of the git Tree, otherwise an
/// Error
Result<gd::ObjectUpdate>
gd::ObjectUpdate::createDir(const std::filesystem::path &fullpath,
                            treebuilder_t &bld) noexcept {
  ObjectUpdate dir{
      create(fullpath, GIT_FILEMODE_TREE, &gd::ObjectUpdate::insert)};
  if (git_treebuilder_write(&dir.oid_, bld) != 0)
    return gd_unexpected();

  sLogger->debug("Create directory '/{}'", fullpath);
  return std::move(dir);
}
...
Result<void>
gd::TreeCollector::removeFile(gd::Context &ctx,
                              const std::filesystem::path &fullpath) noexcept {
...
Result<gd::blob_t> gd::TreeCollector::getBlobByPath(
    const gd::Context &ctx,
    const std::filesystem::path &fullpath) const noexcept {
  ...
  if (dirObjsIdx == dirObjs_.end())
    return gd_unexpected(gd::ErrorType::BadDir, "not found in current context");
  ...
  for (int i = objList.size() - 1; i >= 0; --i) {
    ...
    if (obj.name() == name) {
      if (obj.isDelete())
        return gd_unexpected(gd::ErrorType::Deleted,
                             "File deleted in uncommitted context");
      else
  ...
  return gd_unexpected(gd::ErrorType::NotFound,
                       "No update found in uncommitted context");
}
...
git_oid const *gd::Context::getCommitId() const noexcept {
...
gd::internal::Node::Node(Node &&other) noexcept
    : commit_(std::move(other.commit_)), root_(std::move(other.root_)),
...
/// @return a reference to Node for chained assignments
...
/// @todo Maybe short circuiting the call if commitId == to current (This may
/// require adding git_oid* to state)
///
/// Prerequisites: the git_oid is of type git_commit for an existing commit, or
...
bool gd::cleanRepo(const std::filesystem::path &repoFullPath) noexcept {
  return sGit.cleanRepo(repoFullPath);
}

Result<gd::Context> gd::selectRepository(const std::filesystem::path &fullpath,
                                         const std::string &name) noexcept {
...
Result<gd::Context> gd::ni::selectBranch(gd::Context &&ctx,
                                         const std::string &name) noexcept {
  if (not ctx.repo_)
    return gd_unexpected(gd::ErrorType::MissingRepository, sNoRepositoryError);
...
Result<gd::Context> gd::ni::add(gd::Context &&ctx,
                                const std::filesystem::path &fullpath,
                                const std::string &content) noexcept {
  if (not ctx.repo_)
    return gd_unexpected(gd::ErrorType::MissingRepository, sNoRepositoryError);
...
/// @return On success, the context for continued repository access, otherwise
/// an Error
Result<gd::Context> gd::ni::rm(gd::Context &&ctx,
                               const std::string &fullpath) noexcept {
  if (not ctx.repo_)
    return gd_unexpected(gd::ErrorType::MissingRepository, sNoRepositoryError);
...
/// @param ctx The context used to access the repository
/// @param fullpath Original path of the moved file, including filename.
/// @param toFullPath Destination path of the moved file, including filename.
/// @return On success, the context for continued repository access, otherwise
/// an Error
Result<gd::Context> gd::ni::mv(gd::Context &&ctx, const std::string &fullpath,
                               const std::string &toFullPath) noexcept {
  if (not ctx.repo_)
    return gd_unexpected(gd::ErrorType::MissingRepository, sNoRepositoryError);
...
Result<gd::Context> gd::ni::commit(gd::Context &&ctx, const std::string &author,
  ...
  git_oid commitId;
  {
    ...
    /// The only contention point in need of serialization is updates to the
    /// DAG. i.e. commits
...
/// @param ctx The context used to access the repository
/// @return On success the context for continued chaining, otherwise an error.
Result<gd::Context> gd::ni::rollback(gd::Context &&ctx) noexcept {
  ctx.updates_.clean();
  return std::move(ctx);
}
...
/// @return On success, the context to continue the call chain, otherwise an
/// Error
...
/// @brief Reads content of blob(gitspeak for file), first from uncommitted
/// context, otherwise from repository
/// @param ctx The context used to access the repository
/// @param fullpath The fullpath of the blob
/// @return A string representation of the file's content.
Result<gd::ReadContext>
gd::ni::read(gd::Context &&ctx,
             const std::filesystem::path &fullpath) noexcept {
  ...
  if (!contextBlob && contextBlob.error()._type == gd::ErrorType::Deleted)
    return gd_unexpected(std::move(contextBlob));
...
Result<gd::ReadContext>
gd::ni::readblob(gd::Context &&ctx, git_blob *blob,
                 const std::filesystem::path &fullpath) noexcept {
...
Result<gd::Context> gd::shorthand::getThreadContext() noexcept {
